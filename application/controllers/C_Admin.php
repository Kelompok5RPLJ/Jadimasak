...
class C_Admin extends CI_Controller {
    ...
    public function action_delete($id_barang) {
        $data['barang'] = $this->E_Barang->deletebarang($id_barang);
        redirect('C_Admin/delete_barang');
    }
     public function action_deletefilter($id_barang) {
        $data['barang'] = $this->E_Barang->deletebarang($id_barang);
        redirect('C_Admin/filter_barang');
    }

    public function update_barang($id_barang) {
        $data['barang'] = $this->E_Barang->list_barangterverifikasi($id_barang);
        redirect('C_Admin/filter_barang');
    }
     public function filtermember() {
        $data['user'] = $this->E_User->list_user(); 
        $this->load->view('Admin/v_filtermember',$data);
    }
     public function actionmember($id) {
        $data['user'] = $this->E_User->deletemember($id);
